...
def grid_evaluation_delay_seconds(
    ...
    Material export is always urgent because the controller may need to start
    or increase charging. Large import is urgent only while the controller is
    actively commanding Force Charge; once charging is already off, repeatedly
    evaluating sustained household import faster cannot reduce that import.
    """
    if grid_w is None:
